...
#### 任务名称：空间语义理解——中文空间语义的判别与归因

#### 一、任务简介

语言交际中存在大量的空间语义信息，理解这些信息是非常重要的。著名认知语言学家Jackendoff在其概念语义学理论中也指出空间结构是语言系统的四种基本结构之一（其余三个层面：语音、句法、概念语义）[^Jackendoff]。

通常认为，对空间信息的语义理解不仅需要掌握上下文，还需要具备一定的世界知识或者常识，甚至是超出语言范畴的空间想象能力。那么，应如何测试计算机是否具备理解文本中空间信息的能力？

理解空间信息，不仅体现在能够识别常规、正确的空间信息，还体现在能够识别异常、错误的空间信息。如对于“在四面签一个名字”，人类能够意识到其中存在异常，因为“一个名字”通常不会签在“四面”；又如对于“走过火车下”，人类能够清楚地知道通常不会有人在火车的“下”方走路。这些异常有时体现于少见的词语搭配，有时体现于与上下文信息的矛盾，还有时体现于与常识的冲突等等。对于不同层面的异常原因，我们可能需要进行不同的后续处理，因而在识别异常之后，对于异常原因的解释也很重要。
...
评测任务中的语料主要来源于CCL语料库，涵盖小说、散文、词典等文体。需注意实际使用的文本材料是在原始语料的基础上，替换了具有空间方位意义的词语之后，再进行人工标注和检验后得到的。最终得到7782段有效文本材料，合计86万字。各段材料字数的平均值为110.52，标准差为53.00。这些材料根据性质和任务需要被划分至评测的三个任务的不同数据集中，具体分布情况如表2所示[^材料数量]。

> **表2    各子任务的数据集分布情况**
>
> | 子任务                            | 训练集 | 验证集 | 测试集 | 总计  | 备注                                                         |
> | --------------------------------- | ------ | ------ | ------ | ----- | ------------------------------------------------------------ |
> | 1、中文空间语义正误判断           | 4237   | 806    | 794    | 5837  | 三个数据集之间，所使用的原始语料没有交集，下同。             |
> | 2、中文空间语义异常归因合理性判断 | 8534   | 2985   | 2969   | 14488 | (1)任一数据集所使用的`context`与子任务1的验证集和测试集无交集。(2)训练集使用的`context`与子任务1的训练集有交集。 |
> | 3、中文空间语义判断与归因联合任务 | 0      | 1360   | 1305   | 2665  | (1)不提供训练集。(2)验证集和测试集中使用的`context`与子任务1的相应数据集相同。 |
...
> **表3    归因类型说明**
>
> | 类型         | 内部编号 | 描述                                                         | 形式                                                         | 在子任务2中的数量 |
> | ------------ | -------- | ------------------------------------------------------------ | ------------------------------------------------------------ | ----------------- |
> | 词语搭配问题 | A        | 这种词语搭配在正常文本中几乎不会使用，可能存在更好的表达方式。 | “`[text1]`”和“`[text2]`”不宜搭配                             | 5000+             |
> | 语义问题     | B        | 虽然词语之间可以搭配，但词语之间的语义存在矛盾               | “`[text1]`”和“`[text2]`”语义冲突                             | 4300+             |
> | 语境问题     | C        | 词语之间没有语义上的矛盾，但上下文信息存在矛盾               | “`[text1]`”与上下文“`[text2]`”不和谐                         | 2200+             |
> | 常识问题     | D        | 文本描述的内容不符合常识或常理                               | (1)“`[text1]`”与常识“`[commonsense]`”有一定冲突；(2)“`[text1]`”与常识不符 | 2000+             |
> | 其他问题     | E        | 不好归类为上述类型的其他情形                                 | `[description]`                                              | 500+              |
...
**最终排名**：在所有参赛队伍的评测结果产生之后，计算每个任务下各个队伍的标准分数（Z-score），对三个任务的标准分数取平均，作为最终排名的依据。标准分数计算公式如下，其中 $\bar{X}$ 为平均数， $s$ 为标准差：
$$
Z = \frac{X-\bar{X}}{s}
$$
...
邮件名：`CCL2021-SRU-[队伍名称]-[队伍单位]-[YYDD]-数据提交` ，如 `CCL2021-SRU-博雅队-北京大学-0402-数据提交` 。

文件名：`CCL2021-SRU-[队伍名称]-[队伍单位]-[YYDD]-数据.json` ，如 `CCL2021-SRU-博雅队-北京大学-0402-数据.json` 。

邮件名：`CCL2021-SRU-[队伍名称]-[队伍单位]-[YYDD]-模型提交` ，如 `CCL2021-SRU-博雅队-北京大学-0402-模型提交` 。

文件名：`CCL2021-SRU-[队伍名称]-[队伍单位]-[YYDD]-模型.zip` 如 `CCL2021-SRU-博雅队-北京大学-0402-模型.json` 。
...
4. 如有其他问题，请直接联系评测会务组：`sru_test AT 163 DOT com` 。
...
// C类，语境问题，正负例
  {
    ...
    "reason": "“猜测要他进内堂去”与上下文“便随着二人向外走去”不和谐",
    "judge2": true
  },
  {
    "qID": 4016,
    "context": "过了好一会，只听得脚步细碎，两个中年妇人从花径上走到凉亭外，略略躬身，微笑道：“请新官人进内堂更衣。”石破天不知是什么意思，猜测要他进内堂去，便随着二人向外走去。",
    "reason": "“猜测要他进内堂去”与上下文“只听得脚步细碎”不和谐",
    "judge2": false
  },
...
// E类，其他问题，正负例
  {
    "qID": 610,
    "context": "我从我的沉思中走了出来，去换了些西班牙钱，到卖票处去买了里程车票，起来买了一份昨天的《太阳报》（EISol），一包烟，然后回到安放着我的手提箱的长椅上去。",
    "reason": "没有“起来”的起点。",
    "judge2": true
  },
  {
    "qID": 5098,
    "context": "母亲不能长期地住在乡间，外祖母到镇上来时也只有几天的小住，大概是惯于乡间的安静，不耐街上的烦扰罢，所以亲近外祖母的机会，比亲近祖母的机会少；然而外祖母也和祖母一样，在我底记忆左永远留着活鲜的影子。她真是慈爱底化身，在我，她真象是传说中的观音一样。",
    "reason": "与“乡间”对应的应为整体的街道，而非街市的某一个方位",
    "judge2": false
  }
```
...
[^Jackendoff]: 参看 Jackendoff（2002）著作《Foundations of language: Brain, meaning, grammar, evolution》第1.2、1.5节。
[^材料数量]: 每段材料配合不同归因将会形成不同题目，因此题目数量大于材料数量（7782段）。
